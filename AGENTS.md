...
`src/skill.js` renders the installable Agent Skill from the same home output, keeping the `lavish-axi` commands as-is so a pinned downstream fork can wrap them, omitting live session state, and keeping the frontmatter within the Agent Skills standard (`name`, `description`, `license`, and a `metadata` block carrying `argument-hint`, `author`, and `upstream`) so it stays portable across shared harnesses. The rendered body tells the agent to use the `lavish-axi` command on `PATH` and, if it is missing, to stop and say so rather than fall back to an unpinned `npx -y lavish-axi`.
...
- The in-iframe layout audit (`src/artifact-sdk.js`) has four easy-to-reintroduce failure modes, all covered by dedicated tests:
  ...
  - Fixed-size boxes with `overflow: visible` (the default) - badges, pills, buttons - don't clip overflowing content, they let wrapped text spill out and overlap neighbors, which is just as broken as `overflow: hidden` clipping it. `scrollHeight` alone is not proof: tight single-line font metrics and decorative content can also enlarge it, so `classifyVerticalOverflow` cross-checks real text-range fragments against the padding box and ignores a lone visible line. Because a real visible spill bubbles through unconstrained ancestors, `resolveSpillCandidates` keeps only the innermost element that shares the same spill edge.
  - Child bounds extending beyond a parent's content box do not necessarily indicate broken layout: negative-margin accents can remain inside the parent's scroll geometry. `classifyParentOverflow` ignores overhang without parent scroll impact and keeps the remaining heuristic finding warning-only; document-level horizontal overflow remains the strict error signal.
  - `SessionStore` marks a re-reported layout warning `persistent: true` once its `kind:selector` key has already been delivered to the agent via `takeFeedback` (tracked in `session.delivered_layout_warning_keys`, independent of the `layout_warnings` field that gets cleared on each delivery). `cli.js`'s `createFeedbackNextStep` softens its guidance - permitting the agent to proceed to the human instead of looping fixes and reloads - once every current warning is either `persistent` or below `error` severity (currently the always-warning heuristics `overlapping-text` and `element-parent-overflow`, both of which stay warning-only even after the fragment-aware and scroll-geometry checks).
